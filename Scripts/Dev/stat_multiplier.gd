...
@onready var optional_line_edit = $VBoxContainer/StatNameOptional
...
func _set_multiplier(input, stat_name):
	if input == "0":
		...
		game.player.cheats[stat_name] = num
	
	elif int(input) != 0:
		game.player.cheats[stat_name] = B.new(int(input))


func _on_button_pressed() -> void:
	var input:String = line_edit.text
	var stat_name
	print(optional_line_edit.text)
	if optional_line_edit.text:
		stat_name = optional_line_edit.text
	else:
		stat_name = stat_option.get_item_text(stat_option.selected)

	_set_multiplier(input, stat_name)
